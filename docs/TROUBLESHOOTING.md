...
- [ ] LIFF 詳細 sheet UI + state: `src/app/liff/_components/WatchDetailSheet.tsx`
- [ ] LIFF 新增 sheet (有需要的話): `src/app/liff/_components/AddWatchSheet.tsx`
...
### 設計 tokens
所有 LIFF 共用 iOS dark mode tokens 都集中在 `src/app/liff/_styles/tokens.css`（PR #1 從 SubscriptionsViewV2 inline 區塊提出來）。**改顏色**改這檔即可，全部 LIFF 元件都會同步看到。
